...
export class ElectronIpcService {
  ...
  storage = {
    get: (key: string, subpath?: string[]): Promise<IpcMessage> => new Promise<IpcMessage>((resolve, reject) => {
      let config = {
        key: key,
        subpath: subpath
      }
      ...
      // Listen for response
      this.receive('E2A:Storage:Get', (response: IpcMessage) => {
        if (response.error) {
          reject(response.error.message);
        } else if (response.success?.data) {
          if (typeof response.success.data === 'string') {
            response.success.data = JSON.parse(response.success.data);
          }
          resolve(response);
        } else {
          reject('Invalid storage request via Electron IPC.')
        }
      })

      this.send('A2E:Storage:Get', config);
    }),
    getMany: (key: string[], subpath?: string[]): Promise<IpcMessage> => {
      return new Promise<IpcMessage>((resolve, reject) => {
        ...
        this.receive('E2A:Storage:GetMany', (response: IpcMessage) => {
          if (response.error) {
            reject(response.error.message);
          } else if (response.success?.data) {
            if (typeof response.success.data === 'string') {
        ...
        this.send('A2E:Storage:GetMany', config);
      })
    },
    set: (key: string, data: any, subpath?: string[]): Promise<IpcMessage> => {
      return new Promise<IpcMessage>((resolve, reject) => {
        let config = {
          key: key,
          data: data,
        ...
        this.receive('E2A:Storage:Set', (response: IpcMessage) => {
          if (response.error) {
            reject(response.error.message);
        ...
        this.send('A2E:Storage:Set', config);
      })
    },
    delete: (key: string | string[], subpath?: string[]): Promise<IpcMessage> => {
      return new Promise<IpcMessage>((resolve, reject) => {
        let config = {
          key: key,
          subpath: subpath
        ...
        this.receive('E2A:Storage:Delete', (response: IpcMessage) => {
          if (response.error) {
            reject(response.error.message);
          } else if (response.success) {
            resolve(response);
        ...
        this.send('A2E:Storage:Delete', config);
      })
    }
  }
