...
@Component({
  ...
  template: `
    ...
    <p-dialog [(visible)]="exportDialogVisible" header="Export" [modal]="true">
      <div>
...
export class KsExportComponent {
  ...
  createFile(encoding: string, type: string, charset: string) {
    ...
    if (link.download !== undefined) {
      ...
      link.setAttribute(
        'download',
        this.EXPORT_FILENAME + this.selectedExportType
      );
      link.click();
    } else {
  ...
  export() {
    this.exportTypes
      .find((e) => e.value === this.selectedExportType)
      ?.export(this.data);
  }
  ...
  encodeJSON(data: KnowledgeSource[]): string {
    ...
    for (const ks of data) {
      const temp: any = {};
      const selectedColumns = this.EXPORT_COLUMNS.filter((c) =>
        this.selectedFeatures.includes(c.field)
      );
