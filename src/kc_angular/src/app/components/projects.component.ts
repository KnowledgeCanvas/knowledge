      ...
      <div class="source-topics">
        <p-card header="Top-20 Topics">
          <ng-template pTemplate="content">
            <p-chips
              [ngModel]="topics.asObservable() | async"
...
export class ProjectsComponent implements OnDestroy {
  ...
  project: BehaviorSubject<KcProject | null> =
    new BehaviorSubject<KcProject | null>(null);
      ...
      .pipe(
        ...
        tap((sources: KnowledgeSource[]) => {
          ...
          sources.forEach((ks) => {
            ...
            topics.sort((a, b) => {
              const nA = sources.filter((k) => k.topics?.includes(a)).length;
              const nB = sources.filter((k) => k.topics?.includes(b)).length;
