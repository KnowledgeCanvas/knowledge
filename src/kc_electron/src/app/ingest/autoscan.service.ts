...
import {FileSourceModel, FileWatcherUpdate, PendingFileTransfer} from "../../../../kc_shared/models/file.source.model";
...
class FileManagerService {
    ...
    constructor() {
        ...
        ipcMain.on(FileManagerService.channels.finalize, (event: any, update: FileWatcherUpdate) => {
        ...
        ipcMain.on(FileManagerService.channels.moveToManaged, (_: any, currPath: string) => {
            const filename = path.basename(currPath);
            const storageLocation = settingsService.getSettings().ingest.manager.storageLocation;
    ...
    private static move(from: string, to: string) {
        ...
        try {
            ...
            FileManagerService.error('Exception', `Failed to move file from ${from} to ${to}` + e);
        }
    }
    ...
    private async getLocalStorageItem(key: string) {
        let kcMainWindow = share.BrowserWindow.getAllWindows()[0];
        return kcMainWindow.webContents.executeJavaScript(`localStorage.getItem('${key}');`, true);
    }

    private async setLocalStorageItem(key: string, value: any) {
        const val = JSON.stringify(value);
        let kcMainWindow = share.BrowserWindow.getAllWindows()[0];
        return kcMainWindow.webContents.executeJavaScript(`localStorage.setItem('${key}', '${val}');`, true)
    }
    ...
    private clean(ingest: IngestSettingsModel) {
        this.getPending().then((pending) => {
            ...
            let files;
            try {
                const pendingPath = path.resolve(ingest.manager.storageLocation, 'pending');
            ...
            for (let file of files) {
                ...
                console.log('AutoscanService: Found pending files in autoscan storage location...', file);

                const exists = pending.find(p => p.newPath.includes(filename));
                if (exists) {
                    move.push({from: exists.newPath, to: path.resolve(ingest.autoscan.path, exists.filename)});
                    this.removePending(exists.id);
                } else {
                    const oldPath = path.resolve(ingest.manager.storageLocation, 'pending', file);
    ...
    private async finalize(id: string, method: 'add' | 'remove' | 'delay') {
        ...
        if (!pending) {
            console.log('AutoscanService unable to find pending file transfer... ignoring...');
            return;
        } else {
            switch (method) {
                case "add":
        ...
        this.interval = setInterval(() => {
            ...
            for (let filePath of this.queue) {
                ...
                if (!fileExtension) {
                    ...
                    console.warn(`IngestFileWatcher: Could not find file extension for filePath: ${filePath}... using brute force (${fileExtension}) instead`);
                }

                /* Resolve the new file path, prepend PENDING_PREFIX to the newId to handle life cycle management */
                let appEnv: SettingsModel = settingsService.getSettings();
                let newFilePath = path.resolve(PENDING_DIR(), `${newId}`) + `.${fileExtension}`;
            ...
            kcMainWindow.webContents.send(FileManagerService.channels.newFiles, requests);
            this.queue = [];
    ...
    private async update(settings: SettingsModel) {
        ...
        /* Case: Autoscan already on, but some other setting changed */
            ...
            if (prev.autoscan.interval !== next.autoscan.interval || prev.autoscan.path !== next.autoscan.path) {
                if (prev.autoscan.interval !== next.autoscan.interval) {
                    console.debug(`Autoscan interval changed: ${prev.autoscan.interval} to ${next.autoscan.interval}`,);
                }
                if (prev.autoscan.path !== next.autoscan.path) {
                    console.debug(`Autoscan path changed: ${prev.autoscan.path} to ${next.autoscan.path}`,);
                }
                this.stop();
                this.start(next);
            }
        ...
        if (prev.manager.storageLocation !== next.manager.storageLocation) {
            ...
            FileManagerService.move(prev.manager.storageLocation, next.manager.storageLocation);
